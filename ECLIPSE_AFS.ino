...
#define CTRL7_ACCEL_GYRO   0x03  // aEN=1, gEN=1
...
bool imuInit() {
  ...
  delay(20);  // Wait for reset (max 15ms per datasheet)
...
void imuReadAll() {
  uint8_t buf[14];
  imuReadRegs(REG_TEMP_L, buf, 14);

  // Temperature (regs 0x33-0x34, Little-Endian: L then H)
  int16_t rawTemp = ((int16_t)buf[1] << 8) | buf[0];
  imuTemp = rawTemp * tempScale;

  // Accel (regs 0x35-0x3A)
  int16_t rawAx = ((int16_t)buf[3] << 8) | buf[2];
  int16_t rawAy = ((int16_t)buf[5] << 8) | buf[4];
  int16_t rawAz = ((int16_t)buf[7] << 8) | buf[6];
  accelX = rawAx * accelScale;
  accelY = rawAy * accelScale;
  accelZ = rawAz * accelScale;

  // Gyro (regs 0x3B-0x40)
  int16_t rawGx = ((int16_t)buf[9]  << 8) | buf[8];
  int16_t rawGy = ((int16_t)buf[11] << 8) | buf[10];
  int16_t rawGz = ((int16_t)buf[13] << 8) | buf[12];
  gyroX = rawGx * gyroScale;
  gyroY = rawGy * gyroScale;
  gyroZ = rawGz * gyroScale;
}
